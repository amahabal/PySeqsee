...
class TestStream(unittest.TestCase):
  def test_basic(self):
    ...
    hits_map = s._StoreFringeAndCalculateOverlap(m3)
    ...
    hits_map = s._StoreFringeAndCalculateOverlap(m6)
    ...
    hits_map = s._StoreFringeAndCalculateOverlap(m12)
    ...
    hits_map = s._StoreFringeAndCalculateOverlap(f)
    self.assertEqual(3, s.FociCount())
    self.assertEqual(0, len(hits_map))
