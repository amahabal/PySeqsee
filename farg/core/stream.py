...
class Stream(object):
    ...
    def _remove_prior_focus(self, focusable):
    ...
    def _remove_most_ancient_focus(self):
        """Remove the focus with the least strength."""
        most_ancient_focus = min(self.foci, key=self.foci.get)
        self._remove_prior_focus(most_ancient_focus)

    def _prepare_for_focusing(self, focusable):
        """Does legwork for focusing; if focusable is a recent focus, removes it."""
        # If already stored, delete it.
        if focusable in self.foci:
            self._remove_prior_focus(focusable)

        if self.foci_count() == self.kMaxFocusableCount:
            self._remove_most_ancient_focus()

    def focus_on(self, focusable, *, parents=None):
        ...
        self._prepare_for_focusing(focusable)
        hit_map = self.store_fringe_and_calculate_overlap(focusable)
    ...
    def store_fringe_and_calculate_overlap(self, focusable):
